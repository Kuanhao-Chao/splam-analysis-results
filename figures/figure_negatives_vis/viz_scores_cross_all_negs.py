...
# fig, axes = plt.subplots(2, 1, figsize=(8, 8))  # Adjusting the size for better visibility
...
# axes[0].set_title('Mean Donor Scores by Threshold with Error Bars')
# axes[0].set_xlabel('Spliced Alignment Count')
# axes[0].set_ylabel('Mean Donor Score')
...
# axes[0].plot(thresholds_list, p(thresholds_list), "r--", label='Donor trend line')  # "r--" makes the line red and dashed
# axes[0].legend()

# axes[1].set_title('Mean Acceptor Scores by Threshold with Error Bars')
# axes[1].set_xlabel('Spliced Alignment Counts')
# axes[1].set_ylabel('Mean Acceptor Score')
...
# p = np.poly1d(z)  # Create the polynomial equation based on the fit
# # Plotting the trend line
# axes[1].plot(thresholds_list, p(thresholds_list), "r--", label='Acceptor trend line')  # "r--" makes the 
# axes[1].legend()
...
axes[0].set_title(f'Ratio of Splice Junctions with Donor Scores above Threshold {selected_threshold}')
...
axes[0].set_xlabel('Spliced Alignment Count')
axes[0].set_ylabel('Ratio of Splice Junctions')
axes[0].legend()

axes[1].set_title(f'Ratio of Splice Junctions with Acceptor Scores above Threshold {selected_threshold}')
...
axes[1].set_ylabel('Ratio of Splice Junctions')
axes[1].legend()
