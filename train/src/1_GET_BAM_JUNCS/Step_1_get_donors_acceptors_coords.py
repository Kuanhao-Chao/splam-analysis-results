...
def main():
    ...
    output_dir = f'{project_root}/train/results/BAM_junctions/'+SEQ_LEN+"bp/"+THRESHOLD+'_juncs/'
    os.makedirs(output_dir, exist_ok=True)
    fw_donor = open(f"{output_dir}/donor.bed", "w")
    fw_acceptor = open(f"{output_dir}/acceptor.bed", "w")
    d_a_bed = f"{output_dir}/d_a.bed"
    fw_da = open(d_a_bed, "w")
    JUNCS = set()
    with open(f'{project_root}/train/data/junctions_{str(THRESHOLD)}.bed', "r") as f:
