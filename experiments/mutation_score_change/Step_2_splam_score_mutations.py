...
def parse_original_junction(name):
    # print("name: ", name)
    res = name.split(":")
    strand = name[-2]
    chr_name = res[0]
...
def main():
    ...
    for target in TARGETS:
        output_dir_base = f"../../results/mutation/{target}/"

        if SCORE_ORIGINAL == True:
            intput_seq_dir = f"{output_dir_base}seq/original.fasta"
            output_score_dir = f"{output_dir_base}scores/original/"

        for seq_len in range(800):
            intput_seq_dir = f"{output_dir_base}seq/pos_{seq_len}.fasta"
            output_score_dir = f"{output_dir_base}scores/pos_{seq_len}/"

            os.makedirs(output_score_dir, exist_ok=True)
            d_score_tsv_f = f"{output_score_dir}splam_all_seq.score.d.tsv"
            a_score_tsv_f = f"{output_score_dir}splam_all_seq.score.a.tsv"
            # n_score_tsv_f = f"{output_score_dir}splam_all_seq.score.n.tsv"
            # name_tsv_f = f"{output_score_dir}splam_all_seq.name.tsv"
            d_score_fw = open(d_score_tsv_f, "a")
            a_score_fw = open(a_score_tsv_f, "a")
            # n_score_fw = open(n_score_tsv_f, "a")
            # name_fw = open(name_tsv_f, "a")
            test_loader = get_eval_dataloader(BATCH_SIZE, MODEL_VERSION, N_WORKERS, False, intput_seq_dir)
            # test_iterator = iter(test_loader)
            print(f"[Info]: Finish loading data!", flush = True)
            print("valid_iterator: ", len(test_loader))
            LOG_OUTPUT_TEST_BASE = f"{output_score_dir}/LOG/"
