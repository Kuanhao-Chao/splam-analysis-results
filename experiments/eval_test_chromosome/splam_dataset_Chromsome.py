...
class myDatasetEval(Dataset):
    def __init__(self, type, segment_len=800, shuffle=True, test_f=""):
...
def get_eval_dataloader(batch_size, TARGET, n_workers, shuffle, test_f):
    #######################################
    # predicting splice / non-splice
    #######################################
    testset = myDatasetEval("eval", int(SEQ_LEN), shuffle, test_f)
    test_loader = DataLoader(
        testset,
        batch_size = batch_size,
        shuffle = shuffle,
